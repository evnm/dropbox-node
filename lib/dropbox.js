...
querystring.escape = function( p ) {
    return p.trim().replace(/([!|\:|\*|'|\(|\)|;|@|&|=|\+|\$|,|\/|\?|#|\[|\]|~|\%|\s])/gi, function( char ) {
...
_.extend( Dropbox.prototype, {
    ...
    _OAuthGet : function( uri, params, callback ) {
        ...
        this.OAuth.get( uri, params.accessToken || this.accessToken, params.accessTokenSecret || this.accessTokenSecret, OAuthCallback( callback ));
    },
    ...
    */
    ...
    // user's Dropbox root.
    getFile : function( path, params, cb ) {
    ...
    putFile : function( file, path, params, callback ) {
        ...
        nodePath.exists( file, function( exists ) {
            ...
            if ( exists ) {
                ...
                self._OAuthPost( [ CONTENT_API_URI, 'files', self.boxType, dir ].join("/"), body, contentType, params, callback );
