...
// Uploads data
DropboxClient.prototype.put = function (data, filename, path, optargs, cb) {
	if (typeof optargs == 'function') {
		cb = optargs;
		optargs = {};
	}
	...
	self.oauth.post(CONTENT_API_URI + '/files/dropbox/' + path + '?file=' + filename,
		optargs.token || self.access_token,
		optargs.secret || self.access_token_secret, 
		body, content_type, cb
	);
};

// Uploads data via a stream
// A first chunk needs to be served
DropboxClient.prototype.getPutStream = function (chunk, filename, path, optargs) {
	return DropboxClient.prototype.put(chunk, filename, path, optargs, null);
};
...
DropboxClient.prototype.putFile = function (file, path, optargs, cb) {
	require('fs').readFile(file, function (err, data) {
		if (err) return cb(err);
		DropboxClient.prototype.put(data.toString('binary'), file, path, optargs, cb);
	});
};
