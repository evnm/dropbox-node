...
// Initializes the DropboxClient object.
// If email and password specified in creds, access token/secret pair are
// fetched. Otherwise, key pair specified in creds is used to authenticate
// subsequent calls.
DropboxClient.prototype.init = function(creds, cb) {
  if (typeof creds === 'function') cb = creds, creds = {};
  if (!this.access_token || !this.access_token_secret) {
    if (creds.email && creds.password) {
      var self = this;
      this.oauth.get(API_URI + '/token?' +
                     querystring.stringify({email: creds.email,
    ...
    } else if (creds.token && creds.secret) {
      this.access_token = creds.token;
      this.access_token_secret = creds.secret;
      cb(null);
    } else {
      cb(Error("Insufficient credentials. You must include either an " +
               "email:password or token:secret pair within the credentials " +
